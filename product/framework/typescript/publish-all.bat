@echo off
rem Wrapper that runs publish-all.ps1 with execution policy bypass.
rem ASCII-only comments to avoid cmd.exe encoding issues on Japanese Windows (cp932).
rem All arguments (e.g. -DryRun, -Registry, -MaxParallel, -LogFile) are forwarded to PowerShell.
