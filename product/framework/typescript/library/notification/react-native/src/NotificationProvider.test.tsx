...
describe("NotificationProvider (react-native)", () => {
  ...
  // isConfigEqual の分岐網羅: 未定義遷移 / maxQueueSize / now / idFactory / timer の各単独差分
  // それぞれを単独で変えると 1 回だけ warn が出ることを検証する
  it("warns when isConfigEqual detects a change in any single field", () => {
    ...
    try {
      // 分岐: 片方だけ undefined（undefined → defined）
      expect(runCase(undefined, { defaultDuration: 100 })).toBe(1);
      // 分岐: 片方だけ undefined（defined → undefined）
