...
// React 19 の test renderer 向けに act 環境であることを明示する
(globalThis as { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;

// React 19 でも mount が同期的に完了するよう act 内で描画する
function render(element: ReactElement): ReactTestRenderer {
  // renderer を後で参照できるように保持する
  let renderer: ReactTestRenderer | undefined;
  // mount を act で包む
  act(() => {
    renderer = create(element);
  });
  // act 後には必ず renderer が作成されている
...
// React Native binding の Provider / hook / boundary 動作を検証する
describe("react-native error bindings", () => {
  // Provider の主要経路を検証する
  ...
  it("throws a clear error outside provider", () => {
    ...
    expect(() => {
      // throw を同期的に捕捉できるよう act 内で mount する
      act(() => {
        create(<OutsideProviderHarness />);
      });
