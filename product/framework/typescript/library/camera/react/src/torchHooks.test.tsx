...
describe("useTorch", () => {
  it("初期 mode は 'off' / supported は capabilities 未取得時 false", () => {
  ...
  it("preview-start で capabilities 取得 -> supported=true", async () => {
    const manager = makeManager();
    let captured: ReturnType<typeof useTorch> | undefined;
    function P() {
      captured = useTorch();
  ...
  it("set('on') が成功すれば adapter 呼び出し + mode 楽観更新", async () => {
  ...
  it("set 失敗時は mode を変更せず error を保持", async () => {
  ...
  it("capabilities が torch=false なら supported=false", async () => {
    const manager = makeManager({
