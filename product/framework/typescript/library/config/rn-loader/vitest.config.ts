...
// config rn-loader (RN/RNW 向け) のテスト設定をエクスポート
export default defineConfig({
  // ローカル core ソースを参照してテストする
  resolve: {
    // パッケージ名の解決先を上書きする
    alias: {
      // config core は sibling の src を直接参照する（ローカル変更を即時検証するため）
      "@k1s0-ts-config/core": new URL("../core/src/index.ts", import.meta.url).pathname,
    },
  },
  // テストランナー本体に渡す設定群
  test: {
    ...
    coverage: {
      ...
      include: ["src/**/*.ts"],
      // 除外: テスト本体、型定義のみのファイル、re-export のみの index、fixtures
      exclude: [
        // テストコード自身はカバレッジ対象外
        "src/**/*.test.ts",
