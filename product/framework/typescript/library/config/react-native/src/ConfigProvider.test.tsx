...
// react-native の Platform はデフォルトで ios として扱う（Provider テストでは Platform 依存無し）
...
// ConfigProvider (RN) のテストスイート
describe("ConfigProvider (react-native)", () => {
  ...
  it("children をそのまま描画する", async () => {
    // 識別可能なテキスト
    const marker = "rn-marker-text";
    // 結果を格納する変数
    let tree: ReturnType<typeof create> | undefined;
    // Provider で囲んで描画する
    await act(async () => {
      // 最小 config を渡す（host element として div を渡せば test renderer が JSON 化できる）
      tree = create(
