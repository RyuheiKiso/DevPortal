...
// storage react のテスト設定をエクスポート
export default defineConfig({
  // 解決エイリアス: テスト時に core/src を直接参照する (ビルド成果物に依存しない)
  resolve: {
    alias: {
  ...
  // テストランナー本体に渡す設定群
  test: {
    // DOM API (window / localStorage / StorageEvent / document) を使うので jsdom 環境
    environment: "jsdom",
    // 対象ファイルパターン（src 配下の *.test.ts(x) のみを実行）
    include: ["src/**/*.test.{ts,tsx}"],
    // セットアップ: fake-indexeddb の auto mock 等
    ...
    coverage: {
      ...
      include: ["src/**/*.{ts,tsx}"],
      // 除外: テスト本体、テストランナー設定、re-export のみの index
      exclude: [
        // テストコード自身
        "src/**/*.test.{ts,tsx}",
        // テストランナー設定
