<#
.SYNOPSIS
    product/scaffold 配下の 5 個の YAML (catalog-info × 3 + template × 2) を社内 Backstage に一括登録する。

.DESCRIPTION
    setup ツールで導入された Backstage (http://localhost:7007) の Catalog REST API
    (POST /api/catalog/locations) を使って、scaffold 配下の Location ファイル
    群 (1 System + 2 Component + 2 Template) を登録する。
    依存解決の都合で Round 1 (1 System) → Round 2 (2 Component + 2 Template) の
...
# 登録対象 location 一覧 (Round 1 = System、Round 2 = 2 Component + 2 Template)
# 注: Group / Domain (k1s0-framework-team / k1s0-framework) は framework/typescript の
# register-all.bat で先に登録済みである前提 (owners.yaml は scaffold 配下には作らない)。
$Locations = @(
    # Round 1: 依存先となる System を先に投入する
    [pscustomobject]@{ Name = "system:k1s0-scaffold";                       RelPath = "catalog-info.yaml";                Round = 1 }
    # Round 2: Component (System 解決後に投入)
    [pscustomobject]@{ Name = "component:k1s0-scaffold-react";              RelPath = "react\catalog-info.yaml";          Round = 2 }
    [pscustomobject]@{ Name = "component:k1s0-scaffold-react-native";       RelPath = "react_native\catalog-info.yaml";   Round = 2 }
    # Round 2: Template (Scaffolder の /create/templates に表示される雛形)
    [pscustomobject]@{ Name = "template:k1s0-scaffold-react-template";              RelPath = "react\template.yaml";          Round = 2 }
    [pscustomobject]@{ Name = "template:k1s0-scaffold-react-native-template";       RelPath = "react_native\template.yaml";   Round = 2 }
)
...
function Wait-EntitiesIngested {
    ...
    Write-Log Info "k1s0-scaffold-* エンティティの取り込み完了を待機中 (期待値 $ExpectedCount 件、最大 $MaxSeconds 秒)..."
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            $mine = @($resp | Where-Object { $_.metadata.name -match '^k1s0-scaffold' })
...
function Invoke-ConfigFileMode {
    ...
    if (-not $SkipRestart) {
        ...
        if (-not $token) {
            ...
            $ingested = Wait-EntitiesIngested -BearerToken $token -ExpectedCount 5 -MaxSeconds 90
