...
def main() -> int:
    ...
    # Group / Domain / Template が rules[0].allow に無ければ追記する
    # owners.yaml に Group/Domain、scaffold/<sub>/template.yaml に Template が含まれるため、
    # これらがないと取り込み時 rejected になる。
    allow_list = rules[0].setdefault("allow", [])
    rules_added: list[str] = []
    for kind in ("Group", "Domain", "Template"):
