<#
.SYNOPSIS
    NuGet プライベートレジストリ（BaGetter）をアンインストールする。

.DESCRIPTION
    Setup-BaGetter.ps1 で作成した Windows サービスを停止・削除し、
    インストールディレクトリを削除する。-KeepData を付けると公開済みパッケージと
    DB（data ディレクトリ）を残して本体のみ削除する。
...
.PARAMETER KeepData
    パッケージデータ（data ディレクトリ）を残す。

.PARAMETER Force
    確認プロンプトを表示せずに削除する。

.EXAMPLE
    .\Uninstall-BaGetter.ps1 -Force
#>
[CmdletBinding()]
param(
    # インストール先ディレクトリ
...
Write-Section "BaGetter (NuGet) アンインストール"
...
if (Test-Path $InstallRoot) {
    if ($KeepData) {
        # データディレクトリ以外（app・logs・start.cmd など）を削除する
...
Write-Section "アンインストール完了（BaGetter / NuGet）"
