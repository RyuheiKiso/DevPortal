<#
.SYNOPSIS
    Maven プライベートレジストリ（Reposilite）をアンインストールする。

.DESCRIPTION
    Setup-Reposilite.ps1 で作成した Windows サービスを停止・削除し、
    インストールディレクトリを削除する。-KeepData を付けるとリポジトリの
    データ（公開済みアーティファクト・設定・DB）を残して本体のみ削除する。
...
.PARAMETER KeepData
    リポジトリデータ（data ディレクトリ）を残す。

.PARAMETER Force
    確認プロンプトを表示せずに削除する。

.EXAMPLE
    # 管理者 PowerShell でサービスとファイルを完全削除
    .\Uninstall-Reposilite.ps1 -Force

.EXAMPLE
    # データは残してサービスと本体だけ削除
    .\Uninstall-Reposilite.ps1 -KeepData
#>
[CmdletBinding()]
param(
    # インストール先ディレクトリ
...
Write-Section "Reposilite (Maven) アンインストール"
...
# NSSM のパスを解決する（存在すれば NSSM 経由で削除）
...
if (Test-Path $InstallRoot) {
    if ($KeepData) {
        # データディレクトリ以外（JAR・logs・start.cmd など）を削除する
...
Write-Section "アンインストール完了（Reposilite / Maven）"
